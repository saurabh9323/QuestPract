...
-- Progress, answers, task history, notes and review dates live in this JSONB document.
...
begin
  ...
  insert into public.training_state(user_id,payload,revision)
    values(auth.uid(),new_payload,0) on conflict(user_id) do nothing;
