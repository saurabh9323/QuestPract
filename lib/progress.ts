...
export type Profile = {username?:string;experienceYears?:number;targetRole?:string;targetCompanies?:string;weakAreas?:string[];dailyMinutes?:number;communicationGoal?:string};
...
export function saveCommunication(p:Progress,day:number,patch:Partial<Omit<CommunicationLog,'history'|'updatedAt'>>,now=new Date()):Progress{
 if(!quests[day-1])throw new Error('Unknown communication day.');
 const previous=p.communication?.[day],next={status:patch.status||previous?.status||'planned',notes:patch.notes??previous?.notes??'',reply:patch.reply??previous?.reply??''} as const;
 const changed=previous&&(previous.notes!==next.notes||previous.reply!==next.reply||previous.status!==next.status);
 return {...p,updatedAt:now.toISOString(),communication:{...p.communication,[day]:{...next,updatedAt:now.toISOString(),history:changed?[...(previous.history||[]),{status:previous.status,notes:previous.notes,reply:previous.reply,savedAt:previous.updatedAt}]:previous?.history||[]}}};
}
...
export function validateProgress(input:unknown):Progress{
 ...
 if(p.profile){if(typeof p.profile!=='object'||Array.isArray(p.profile)||(p.profile.username!==undefined&&(!/^[a-zA-Z0-9_]{0,30}$/.test(p.profile.username)))||(p.profile.experienceYears!==undefined&&(typeof p.profile.experienceYears!=='number'||p.profile.experienceYears<0||p.profile.experienceYears>50))||(p.profile.targetRole!==undefined&&(typeof p.profile.targetRole!=='string'||p.profile.targetRole.length>120))||(p.profile.targetCompanies!==undefined&&(typeof p.profile.targetCompanies!=='string'||p.profile.targetCompanies.length>300))||(p.profile.communicationGoal!==undefined&&(typeof p.profile.communicationGoal!=='string'||p.profile.communicationGoal.length>1000))||(p.profile.dailyMinutes!==undefined&&(!Number.isInteger(p.profile.dailyMinutes)||p.profile.dailyMinutes<15||p.profile.dailyMinutes>480))||(p.profile.weakAreas!==undefined&&(!Array.isArray(p.profile.weakAreas)||p.profile.weakAreas.length>8||p.profile.weakAreas.some(x=>typeof x!=='string'||x.length>60))))throw new Error('Invalid profile settings.')}
 ...
 if(p.oop){
  if(typeof p.oop!=='object'||Array.isArray(p.oop)||Object.keys(p.oop).length>100)throw new Error('Invalid OOP progress.');
  for(const [id,r] of Object.entries(p.oop)){
   if(!/^[a-z][a-z0-9-]{0,59}$/.test(id)||!r||typeof r.draft!=='string'||r.draft.length>30000||!stamp(r.updatedAt)||!Array.isArray(r.attempts)||r.attempts.length>200||(r.reviewDue!==undefined&&!validDate(r.reviewDue)))throw new Error('Invalid OOP record.');
